...
| `src/governance_tools/` | The package: `control` (the checked control value and its per-target overrides), `baseline` (load and validate), `gh` (the only IO), `compare` (pure comparison and the stricter guard), `controls` (per-control read/apply), `check` (per-control classification), `bootstrap`, `org` and `audit` (orchestration), `matrix` and `report` (rendering). |
